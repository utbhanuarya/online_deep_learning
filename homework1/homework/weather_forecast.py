...
class WeatherForecast:
    ...
    def find_the_largest_drop(self) -> torch.Tensor:
        ...
        day_over_day_change = torch.diff(daily_means)
        return torch.min(day_over_day_change) 
    ...
    def find_the_most_extreme_day(self) -> torch.Tensor:
        ...
        daily_means = torch.mean(self.data, dim=1).unsqueeze(1)
        
        # Calculate absolute differences from mean for each measurement
        differences = torch.abs(self.data - daily_means)
        
        # Find the indices of measurements with maximum difference for each day
        max_diff_indices = torch.argmax(differences, dim=1)
        
        # Get the actual temperature values at these indices
        result = torch.tensor([self.data[i][idx] for i, idx in enumerate(max_diff_indices)])
    ...
    def predict_temperature(self, k: int) -> torch.Tensor:
        ...
        """
        last_k_days = self.data[-k:]
        return torch.mean(torch.mean(last_k_days, dim=1))
    ...
    def what_day_is_this_from(self, t: torch.FloatTensor) -> torch.LongTensor:
        ...
        """
        # Calculate the sum of absolute differences between t and each day's measurements
        sad = torch.sum(torch.abs(self.data - t), dim=1)
        
        # Find the index of the day with the smallest SAD
        closest_day_index = torch.argmin(sad)
